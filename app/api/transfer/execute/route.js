...
export async function POST(request) {
    ...
    const { source, dest, playlistIds } = body;
    // const dest = source === 'spotify' ? 'youtube' : 'spotify'; // No longer inferred
    const cookieStore = await cookies();
    const sourceToken = cookieStore.get(`${source}_source_access_token`)?.value;
    const destToken = cookieStore.get(`${dest}_dest_access_token`)?.value;
